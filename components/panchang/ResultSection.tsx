...
function mergeAdjacent(slots: BusinessSlot[]): BusinessSlot[] {
  ...
  for (const slot of sorted) {
    ...
    // Only stitch genuinely-identical adjacent windows (e.g. the early-morning /
    // current-day seam at sunrise). Without the Panchang check, the now-gapless
    // day (inauspicious periods are ignored) would collapse into one huge slot.
    if (
      prev && Math.abs(prev.end - slot.start) <= 60_000 &&
          ...
          {slots.length > 5 && (
            <button
              ...
              style={{
                ...
                color: 'var(--moonsilver-dim)', fontFamily: 'Cinzel, serif', fontSize: '0.68rem',
                letterSpacing: '0.08em', padding: '0.2rem 0', display: 'flex', alignItems: 'center', gap: '0.3rem',
              }}
            >
